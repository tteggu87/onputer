...
export function createTools(config: Config, jobs: Jobs) {
  const server = new McpServer({ name: 'onputer', version: '0.1.0' }, { instructions: 'Start with open_workspace. Read AGENTS.md instructions for each target path and relevant skills before editing. Use expected_hash from read_file for edits. Commands run as the local OS user. Check process exit_code and git_changes before reporting success.' });
  ...
  tool('server_status', 'Show server OS, default shell and enabled capabilities. No tokens are returned.', {}, true, () => ({ version: '0.1.0', platform: process.platform, default_shell: process.platform === 'win32' ? 'powershell' : process.platform === 'darwin' ? 'zsh' : 'sh', commands_enabled: config.allowCommands, workspace_count: config.roots.length }));
